...
const PAGE_SIZE = 15;
const PIE_COLORS = [
  "#6d4fe8", "#1253c4", "#0a7a40", "#b87200", "#c4233e", "#0ea5e9",
  "#7c3aed", "#0891b2", "#15803d", "#d97706", "#be123c", "#0284c7",
  "#6366f1", "#059669", "#ea580c", "#7e22ce", "#0e7490", "#dc2626",
  "#4f46e5", "#16a34a", "#ca8a04", "#9333ea", "#0369a1", "#b91c1c",
];
...
export function App() {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (payload) => {
      setUploadMessage(`${payload.message} (${payload.queuedBatches} batch queued)`);
  ...
  const pieSegments = useMemo(() => {
    const total = topCategories.reduce((sum, item) => sum + Math.abs(item.total), 0);
    if (total === 0) return [];
      ...
      <main className="dashboard">
        <header className="header">
          ...
          <p className="subtitle">
            Upload statements, track spending, and monitor processing.
            <span className={`live-pill ${workerIsLive ? "is-live" : "is-idle"}`}>
        ...
        <section className="upload-panel upload-panel--row">
              ...
              <tbody>
                {(transactionsQuery.data?.data ?? []).map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span className={`amount-pill ${item.amount < 0 ? "negative" : "positive"}`}>
